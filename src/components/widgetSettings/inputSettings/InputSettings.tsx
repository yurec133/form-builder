...
import {
    ...
    updateHideSettingsInput, updateFocusedSettingsInput,
...
const InputSettings = () => {
  const { openWidget, isWidget:{isWidgetInput}, idItem }: IWidgetState = useSelector(
    (state: IStateWidget) => state.widget
  );
  const { label, color, margin, size, variant, isHideField, focused } = useSelector(
    (state: IStateWidget) =>
  ...
  if (!openWidget || !isWidgetInput) return null;
    ...
    <>
      ...
      <Box mb={2}>
        <SelectSetting
          ...
          title={"Size"}
          value={size}
          dispatchUpdate={updateSizeSettingsInput}
          menuItem={memoSize}
        />
      </Box>
      <Box mb={2}>
